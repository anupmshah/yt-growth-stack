...
function runView(value: unknown) { const row=object(value); const input=object(row.input); return { id: row.id, projectId: row.project_id, conversationId: row.conversation_id, status: row.status, provider: row.provider, providerJobId: row.provider_job_id, target: input.target ?? input.channelUrl ?? input.url ?? (Array.isArray(input.channels) ? input.channels[0] : undefined) ?? (Array.isArray(input.urls) ? input.urls[0] : undefined), budgetCents: row.budget_cents, sourceCount: Array.isArray(object(row.result).evidence) ? (object(row.result).evidence as unknown[]).length : undefined, startedAt: row.started_at, finishedAt: row.finished_at, cancelledAt: row.cancelled_at, error: row.error, createdAt: row.created_at, updatedAt: row.updated_at }; }
