...
export const realtimeTools = [
  ...
  { type: "function", name: "create_research", description: "Start bounded channel-level YouTube research for approval. For Apify, pass one or more channel URLs or @handles; never ask for individual video links. It discovers videos from each channel, defaults to 10 videos, and accepts at most 25 channels/500 videos. Firecrawl accepts at most 100 web URLs.", parameters: { type: "object", properties: { provider: { enum: ["apify", "firecrawl"] }, channels: { type: "array", items: { type: "string" } }, urls: { type: "array", items: { type: "string" } }, maxVideos: { type: "number" } }, required: ["provider"] } },
  ...
  { type: "function", name: "collect_research_evidence", description: "Collect completed provider output, normalize sources, and generate conservative evidence-backed opportunities.", parameters: { type: "object", properties: { researchId: { type: "string" } }, required: ["researchId"] } },
